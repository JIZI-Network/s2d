...
async fn transfer(
        ...
        match field.content_type() {
            Some(content_type) => {
                    ...
                    && content_type.subtype() == mime::OCTET_STREAM
                {
                    ...
                    let content_disposition = match field.content_disposition() {
                        None => {
                            return HttpResponse::BadRequest().body("Missing content disposition")
                        }
                    ...
                    while let Some(chunk) = field.next().await {
                        let chunk = match chunk {
                            Ok(chunk) => chunk,
                            Err(_) => {
                                return HttpResponse::BadRequest().body("Failed to read chunk")
                            }
                        };
            ...
            None => {
                ...
                let data = match std::str::from_utf8(bytes.as_ref()) {
                    Ok(data) => data,
                    Err(_) => return HttpResponse::BadRequest().body("Failed to read data"),
                };
    ...
    let http = Http::new("");
    let webhook_url = match config.webhook.url.get(form_data["to"].as_str()) {
        Some(url) => url,
        None => return HttpResponse::BadRequest().body("Missing to"),
    };
    let webhook = match Webhook::from_url(&http, webhook_url).await {
        Ok(webhook) => webhook,
        Err(_) => return HttpResponse::BadRequest().body("Invalid webhook"),
    };
    ...
    match webhook.execute(&http, false, builder).await {
        ...
        Err(_) => HttpResponse::BadRequest().body("Failed to execute webhook"),
    }
}
